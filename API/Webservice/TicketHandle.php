...
<?php
  ...
  // count is the listing of numbers in ticketdashboard
	...
	//$sender = "1001";
			...
			$result = mysqli_query( $sqlconn,"UPDATE `mecacomt_mobiradeon`.`ticket` SET `TIC_Status` = 'PENDING',  `Admin_ID` = '$sender' WHERE `ticket`.`TIC_ID` = '$TicketID';");
	...
	//$sender = "1001";
			...
			$result = mysqli_query( $sqlconn,"UPDATE `mecacomt_mobiradeon`.`ticket` SET `TIC_Status` = 'SOLVED' WHERE `ticket`.`TIC_ID` = '$TicketID';");
		...
		$check = 1;
			while($row = mysqli_fetch_assoc($dataquery)) {
				    ...
				    if($acz == "USER"){
						if ( ($row["TIC_Status"] == "NEW") && ($row["User_ID"] == $sender)  ){ $NEWt++;}
						if ( ($row["TIC_Status"] == "PENDING") && ($row["User_ID"] == $sender)  ){ $Pending++;}
						if ( ($row["TIC_Status"] == "PENDING")&& ($row["User_ID"] == $sender) ){ $OPending++;}
