...
// ##############config##############
   ...
   $action = $_GET['action'];
	...
	function loginStuff($data, $sqlconn){
		...
		$output = array('status' => true, 'ID' => $set[0]["User_ID"]  , 'ITICKET' => $set[0]["TIC_ID"] , 'NICK' => $set[0]["User_Nick"] , 'SUBJECT' => $set[0]["TIC_Subject"] , 'ACCESS' => $set[0]["User_Access"],  'STATUS' => $set[0]["User_Status"] );
